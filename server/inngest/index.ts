...
const checkLowStock = inngest.createFunction(
    ...
    async ({ event, step }) => {
      ...
      await step.run("send-low-stock-email", async () =>{
        ...
        if(adminEmails.length === 0 ) return {skipped: true,
            ...
            <div style="font-family: Segoe UI, Arial, sans-serif; max-width: 520px; margin: auto; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
                ...
                <div style="padding: 28px;">
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <div>
                            <h3 style="margin: 10px 0 4px; font-size: 18px; color: #111827;">
                                ${product.name}
                            </h3>
...
const sendMonthlyOffers = inngest.createFunction({
    ...
    const { deals, users } = await step.run(
      ...
      async () => {
        ...
        const allUsers = await prisma.user.findMany({select: {name:
            true, email: true}})
        ...
        // Send in batches of 10 to avoid overwhelming mail serve
...
// Find active riders currently busy with orders
